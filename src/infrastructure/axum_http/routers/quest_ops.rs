use crate::application::usecases::quest_ops::QuestOpsUseCase;
...
use crate::infrastructure::postgres::repositories::quest_ops::QuestOpsPostgres;
...
pub async fn edit<T1, T2>(
    State(quest_ops_use_case): State<Arc<QuestOpsUseCase<T1, T2>>>,
    Path(quest_id): Path<i32>, Json(edit_quest_model): Json<EditQuestModel>)
    -> impl IntoResponse
where
    T1: QuestOpsRepository + Send + Sync,
    T2: QuestViewingRepository + Send + Sync,
...
pub async fn remove<T1, T2>(
    State(quest_ops_use_case): State<Arc<QuestOpsUseCase<T1, T2>>>,
    Path(quest_id): Path<i32>, Extension(guild_commander_id): Extension<i32>)
    -> impl IntoResponse
where
    T1: QuestOpsRepository + Send + Sync,
    T2: QuestViewingRepository + Send + Sync,
